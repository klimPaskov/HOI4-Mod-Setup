...
9. Record commit, manifest hash, manifest origin, platform, sizes, file hashes,
   and the manifest's exact `wiki.required_pages` list.
...
starting its manifest-derived route.
...
The source checkout's committed manifest declared an older generator revision
than the resolved `main` commit. A deterministic local working-tree patch has
now regenerated it from tracked bytes at the exact current revision, but the
remote branch remains unchanged until that patch is reviewed and published.
The application therefore accepts the remote manifest only when its declared
generator revision matches the resolved commit. Until publication, it may use
the bundled manifest only when that bundled file is generated from the same
exact revision; the plan and lock record
`manifest_origin: bundled_revision_bootstrap` and the exact
`wiki_required_pages` list in that case. Readiness never compares a pinned
install with a newer application bundle. This is a temporary publication
bridge, not permission to self-hash downloaded bytes or mix revisions.
