...
if (!existsSync(inputRoot) || !(await stat(inputRoot)).isDirectory()) throw new Error(`release artifact directory does not exist: ${inputRoot}`);
await rm(outputRoot, { recursive: true, force: true });
await mkdir(outputRoot, { recursive: true });
const inputEntries = (await readdir(inputRoot, { withFileTypes: true })).filter((entry) => entry.isDirectory()).map((entry) => entry.name).sort();
const expectedNames = [...expected.keys()].sort();
if (inputEntries.length !== expectedNames.length || inputEntries.some((name, index) => name !== expectedNames[index])) throw new Error(`release artifact set must contain exactly ${expectedNames.join(", ")}; found ${inputEntries.join(", ")}`);
...
function assertSafeRelativePath(path, label) { if (typeof path !== "string" || !path || path.startsWith("/") || path.includes("\\") || path.split("/").includes("..")) throw new Error(`${label} contains an unsafe relative path`); }
function digest(bytes) { return createHash("sha256").update(bytes).digest("hex"); }
function publicPackageName(descriptor) { const target = `${descriptor.platform}-${descriptor.architecture.toLowerCase()}`; return descriptor.platform === "windows" ? `HOI4-Mod-Setup-${target}-setup.exe` : `HOI4-Mod-Setup-${target}.dmg`; }
...
for (const artifactDirectory of expectedNames) {
  ...
  if (metadataFiles.length !== 1 || manifestFiles.length !== 1 || noticeFiles.length !== 1) throw new Error(`${artifactDirectory} must contain one root BUILD_METADATA.json, ARTIFACTS.sha256, and THIRD_PARTY_NOTICES.md`);
  const metadata = JSON.parse(await readFile(metadataFiles[0].absolute, "utf8"));
  if (metadata.product !== "HOI4 Mod Setup" || metadata.platform.toLowerCase() !== descriptor.platform || metadata.architecture.toUpperCase() !== descriptor.architecture) throw new Error(`${artifactDirectory} metadata does not match its expected platform and architecture`);
  if (metadata.signing !== "configured") throw new Error(`${artifactDirectory} is not signed for publication`);
  if (releaseRevision && metadata.sourceRevision?.toLowerCase() !== releaseRevision) throw new Error(`${artifactDirectory} metadata source revision does not match GITHUB_SHA`);
  if (releaseTag && metadata.version !== releaseTag.replace(/^v/, "")) throw new Error(`${artifactDirectory} metadata version does not match the release tag`);
  ...
  for (const entry of manifest) {
    ...
    if (!file || digest(await readFile(file.absolute)) !== entry.sha256.toLowerCase()) throw new Error(`${artifactDirectory} artifact hash mismatch: ${entry.path}`);
  }
  const sourceFilePaths = files.filter(({ relative: path }) => path !== "ARTIFACTS.sha256").map(({ relative: path }) => path);
  if (manifestPaths.size !== sourceFilePaths.length || sourceFilePaths.some((path) => !manifestPaths.has(path))) throw new Error(`${artifactDirectory} artifact manifest does not cover the complete artifact directory`);
  ...
  if (evidence.source_revision?.toLowerCase() !== metadata.sourceRevision?.toLowerCase() || evidence.platform !== metadata.platform || evidence.architecture !== metadata.architecture || evidence.package_sha256?.[packageRelative] !== packageDigest) throw new Error(`${artifactDirectory} signing evidence does not bind the verified package to its metadata`);
  ...
  if (existsSync(sbomDestination) && digest(await readFile(sbomDestination)) !== digest(await readFile(sbomFiles[0].absolute))) throw new Error("platform SBOM inventories do not match");
  ...
  if (existsSync(publicationNotice) && digest(await readFile(publicationNotice)) !== digest(await readFile(noticeFiles[0].absolute))) throw new Error("platform third-party notice inventories do not match");
  if (!existsSync(publicationNotice)) await cp(noticeFiles[0].absolute, publicationNotice);
  platformSummaries.push({ artifact_directory: artifactDirectory, platform: descriptor.platform, architecture: descriptor.architecture, source_revision: metadata.sourceRevision, package: packageName, package_sha256: digest(await readFile(resolve(outputRoot, packageName))), metadata: metadataName, manifest: manifestName, signing_evidence: evidenceName });
}
await writeFile(resolve(outputRoot, "RELEASE_PROVENANCE.json"), JSON.stringify({ schema_version: "1.0.0", product: "HOI4 Mod Setup", source_revision: releaseRevision ?? "unresolved-local", release_tag: releaseTag ?? null, sbom: "SBOM.cdx.json", artifacts: platformSummaries, generated_by: "scripts/prepare_release_assets.mjs" }, null, 2) + "\n", "utf8");
const downloadBase = releaseTag ? `https://github.com/klimPaskov/HOI4-Mod-Setup/releases/download/${encodeURIComponent(releaseTag)}` : null;
const downloadRows = platformSummaries.map((summary) => {
  const label = summary.platform === "windows" ? "Windows x64" : summary.architecture === "ARM64" ? "macOS Apple silicon" : "macOS Intel";
...
const verificationLinks = downloadBase ? `[RELEASE_PROVENANCE.json](${downloadBase}/RELEASE_PROVENANCE.json), [RELEASE_ARTIFACTS.sha256](${downloadBase}/RELEASE_ARTIFACTS.sha256), [SBOM.cdx.json](${downloadBase}/SBOM.cdx.json), and [THIRD_PARTY_NOTICES.md](${downloadBase}/THIRD_PARTY_NOTICES.md)` : "`RELEASE_PROVENANCE.json`, `RELEASE_ARTIFACTS.sha256`, `SBOM.cdx.json`, and `THIRD_PARTY_NOTICES.md`";
...
console.log(`Prepared ${outputFiles.length} uniquely named release assets for ${platformSummaries.length} platforms.`);
