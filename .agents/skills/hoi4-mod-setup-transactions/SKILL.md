...
- Stage 10 may execute a core-owned post-install action runner only for actions
  ...
  Journal redacted bounded action evidence before readiness. Persist a
  separate intent and result checkpoint for each selected external component;
...
- Existing-project ChatGPT source packaging is a separate read-only export, not
  a project transaction. A scan-detected AGENTS, skill, or subagent makes it
  eligible without an installation lock. It defaults to the validated
  Downloads folder, re-reads selected root/skill/subagent files, rejects stale
  IDs, links, traversal, collisions, limits, and secrets, then writes a new
  external ZIP atomically without overwriting or changing the project. Do not
  create a project journal, lock update, or backup for it.
...
- Validate a launcher descriptor's complete `path=` value against the same
  ...
  weaken either platform to a prefix or suffix match. Canonicalize the root
  before rendering so macOS system aliases such as `/var` and `/private/var`
  cannot make staging and readiness disagree with generated bytes.
...
- When the MCP component is selected and the reviewed manifest supplies the
  exact package integrity, full package-tree identity, runtime entry, signed
  Node boundary, and required tools, the reviewed external action performs one
  bounded initialize and read-only `tools/list` probe after apply. Persist its
  result before readiness; a verified route failure blocks the success lock
  and readiness never launches a second probe. When required evidence is absent, the action is
...
- Update planning requires a fresh confirmed selected-provider semantic reanalysis record from the current core session. The UI must first complete a bounded read-only scan, show the approved evidence manifest, run the existing-project common-schema turn, and pass only the core-confirmed record into `build_maintenance_plan`. The record is bound to the canonical project root, latest scan ID, evidence-manifest SHA-256, provider, model, reasoning effort, optimization profile, exact source revision, and exact source-manifest SHA-256. Maintenance reanalysis derives its source request from the installed lock. A legacy record may inherit those two source fields only from valid provenance already stored in the same lock; missing or malformed provenance remains blocked. Repair, reinstall, and managed removal may continue from the validated locked analysis according to their authenticated/recovery rules.
