...
Show a newer application version immediately in one compact, keyboard-
accessible title-bar status and start the signed installation automatically.
Keep current and offline checks silent; after failure show one Retry update
action and never block the wizard on updater state.
